...
def SolveEachLayerSet(X1, X2, X3):
    ...
    layer = [x11, x31, x51, x12, x32, x52, x13, x33, x53]

    box_tmpType = []
    box_usedType = []

    for i in layer:
        box_tmpType.append((abs(round(i.solution_value()))))

    box_usedType.append([box_tmpType[0], box_tmpType[1], box_tmpType[2]])
    box_usedType.append([box_tmpType[3], box_tmpType[4], box_tmpType[5]])
    box_usedType.append([box_tmpType[6], box_tmpType[7], box_tmpType[8]])

    print(box_usedType)
    
    A = [[[2, 0, 0], [[8, 4], [8, 12]]],
         [[0, 4, 0], [[4, 4], [4, 12], [12, 4], [12, 12]]],
         [[0, 0, 8], [[4, 2], [4, 6], [4, 10], [4, 14], [12, 2], [12, 6], [12, 10], [12, 14]]],
         [[1, 2, 0], [[8, 4], [4, 12], [12, 12]]],
         [[1, 0, 4], [[8, 4], [4, 10], [4, 14], [12, 10], [12, 14]]],
         [[0, 1, 6], [[4, 4], [4, 10], [4, 14], [12, 2], [12, 6], [12, 10], [12, 14]]],
         [[0, 2, 4], [[4, 4], [4, 10], [4, 14], [12, 4], [12, 10], [12, 14]]],
         [[0, 3, 2], [[4, 4], [4, 12], [12, 4], [12, 10], [12, 14]]],
         [[1, 1, 2], [[8, 4], [4, 12], [12, 10], [12, 14]]]]

    
    boxPosition = []

    for i in box_usedType:
        for datas in A:
            if datas[0] == i:

                boxPosition.append(datas[1])
    #print("================================")
    #print(boxPosition)
...
class selectBox:
    ...
    def __init__(self):
        self.Subscriber = rospy.Subscriber("/boxList", UInt16MultiArray, self.callback)
    ...
    def callback(self, data):
        ...
        leftBox = [selectBox.X1 - boxCanAlign[0], selectBox.X2 - boxCanAlign[1], selectBox.X3 - boxCanAlign[2]]
        ...
        boxPosition = SolveEachLayerSet(boxCanAlign[0],boxCanAlign[1],boxCanAlign[2])
        print(boxPosition)

        for i in range(len(boxPosition)):
            for j in range(len(boxPosition[i])):
                eachListData = copy(boxPosition[i][j])
                eachListData.insert(0,i+1)

                pubData = UInt16MultiArray(data=eachListData)
                pub.publish(pubData)
                print(eachListData)
